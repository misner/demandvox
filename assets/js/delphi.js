...
/********************************************************************
 * Identify when outer page is not at the bottom and show 'scroll to bottom' btn if that's the case
 ********************************************************************/
function isOuterPageAtBottom(tolerancePx = 2) {
  ...
  return scrollTop + viewportH >= docH - tolerancePx;
}

/**
 * Creates a sentinel element right AFTER the iframe (in the parent page).
 * We observe whether that sentinel is visible in the viewport.
 * If it is NOT visible, user is not at the bottom area.
 */
function ensureSentinelAfterIframe(iframe) {
  const id = "dv-bottom-sentinel";
  ...
  // Keep it tiny, but observable
  sentinel.style.cssText = "width:1px;height:1px;";

  // Insert right after the iframe so it represents "page bottom near composer"
  iframe.insertAdjacentElement("afterend", sentinel);
  return sentinel;
}
